...
import com.femtoapp.expressterml.adapter.account.IncomeRecyAdapter;
...
public class IncomeDtdFragment extends BaseFragment {

    private static final String TAG = "IncomeDtdFragment";
    private RecyclerView rv_recyclerView;
    private IncomeRecyAdapter mIncomeRecyAdapter;
    ...
    public static IncomeDtdFragment newInstance(int type) {
        IncomeDtdFragment fragment = new IncomeDtdFragment();
                    ...
                    if (pay.getStatus() == 0) {
                        if (payBeanList != null) {
                            for (Pay p : payBeanList) {
                                Pay pay1 = new Pay(p.getMoney(), p.getSr_time(), p.getSr_event(), p.getSr_status());
                                mPayList.add(pay1);
                            }
                        }
                        ...
                        getActivity().runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                mIncomeRecyAdapter = new IncomeRecyAdapter(mPayList);
                                rv_recyclerView.setAdapter(mIncomeRecyAdapter);
                                if (mPayList.size() == 0) {
