...
public class ForgetPswActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        time = new TimeCount(60000, 1000); //60s
    }
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ForgetPswActivity.this, "请求失败，请重试！", Toast.LENGTH_SHORT).show();
                    }
                });
            }
                ...
                if (response.isSuccessful()) {
                    ...
                    if (code.getStatus() != 0) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(ForgetPswActivity.this, "验证码错误！", Toast.LENGTH_SHORT).show();
                            }
                        });
                    } else {
                        OkHttpClient client = new OkHttpClient();
                        Request request = new Request.Builder()
                                .get()
                                .url(api.PSWChange1 + "?TELPHONE=" + telephone + "&PASSWORD=" + psw + "&PASSWORD1=" + psw2 + "&code=" + code1)
                                .build();
                        client.newCall(request).enqueue(new Callback() {
                            @Override
                            public void onFailure(Call call, IOException e) {
                                ...
                                if (response.isSuccessful()) {
                                    ...
                                    if (user.getResult() != 0) {
                                        runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                Toast.makeText(ForgetPswActivity.this, user.getResult_data().getERR_MSG(), Toast.LENGTH_SHORT).show();
                                            }
                                        });
                                    } else {
                                        runOnUiThread(new Runnable() {
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(ForgetPswActivity.this, "请求获取验证码失败，请重试！", Toast.LENGTH_SHORT).show();
                    }
                });
                Log.e(TAG, "失败---");
            }
                ...
                if (response.isSuccessful()) {
                    ...
                    if (code.getStatus() != 0) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(ForgetPswActivity.this, "获取验证码失败，请重试！", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
